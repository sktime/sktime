...
test: ## Run unit tests
	...
	python -m pytest -m "not datadownload"
...
test_softdeps_full: ## Run all non-suite unit tests without soft dependencies
	...
	cp setup.cfg ${TEST_DIR}
	cd ${TEST_DIR}
	python -m pytest -v --showlocals --durations=20 -k 'not TestAll' -m 'not datadownload' $(PYTESTOPTIONS)

test_datadownload: ## Run tests for downloads from external sources
	-rm -rf ${TEST_DIR}
	mkdir -p ${TEST_DIR}
	cp setup.cfg ${TEST_DIR}
	cd ${TEST_DIR}
	python -m pytest -v --showlocals -m 'datadownload' $(PYTESTOPTIONS)
