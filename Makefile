...
test_softdeps: ## Run unit tests to check soft dependency handling in estimators
	...
	cp setup.cfg ${TEST_DIR}
	cd ${TEST_DIR}
	python -m pytest -v --showlocals -k 'test_all_estimators' $(PYTESTOPTIONS) --pyargs sktime.registry
	python -m pytest -v --showlocals -k 'test_check_estimator_does_not_raise' $(PYTESTOPTIONS) --pyargs sktime.utils.tests
	python -m pytest -v --showlocals $(PYTESTOPTIONS) --pyargs sktime.tests.test_softdeps
