...
This document formalizes string identifiers used in `.ts` file format. Such files can be opened via any basic
...
6. Finally, save the file as ``<CHOOSE_NAME>.ts``, ensuring that ``save as type`` is set to ``All files``, if present.
