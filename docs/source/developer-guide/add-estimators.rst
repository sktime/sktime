.. _developer-guide_add-estimators:
...
4.  run the ``sktime`` test suite and/or the ``check_estimator`` utility (see `here <https://www.sktime.net/en/latest/developer-guide/add-estimators.html#using-the-check-estimator-utility>`__)
...
*   filling in the tags for the estimator. Some tags are "capabilities", i.e., what the estimator can do, e.g., dealing with nans.
    ...
    The type strings can be found in ``datatypes.MTYPE_REGISTER``. For a tutorial on data type conventions, see ``examples/data-formats``.
...
*   ensure that code also meets ``sktime's`` :ref:`documentation <developer-guide_documentation>` standards.
*   add the estimator to the ``sktime`` API reference. This is done by adding a reference to the estimator in the
    correct ``rst`` file inside ``docs/source/api-reference``.
