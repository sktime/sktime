...
.. code-block::
   ...
   def test_fit_returns_self(object_instance, scenario):
...
This test constitutes a loop over ``object_instance`` and ``scenario`` fixtures,
...
if they use a fixture name (such as ``object_instance``) which already has a loop defined.
...
* ``object_instance`` over estimator instances, obtained from all ``sktime`` estimators via ``create_test_instances_and_names``, which
  constructs instances from parameter settings in estimator classes' ``get_test_params``.

* ``scenario`` objects, which encodes data inputs and method call sequences to ``object_instance`` (explained in further detail below).

The ``sktime`` plug-in ensures that only those ``scenarios`` are retrieved that are
applicable to the ``object_instance``.

In the example, the ``scenario.run`` command is equivalent to calling ``object_instance.fit(**scenario_kwargs)``,
...
The reason for this is that the applicable scenarios (fixture values of ``scenario``) depend on the ``object_instance`` fixture,
...
a ``mark.parameterize`` based on the test arguments (``object_instance`` and ``scenario`` in the above example).
...
* ``object_instance``: all estimator test instances, obtained from all ``sktime`` estimators via ``create_test_instances_and_names``
* ``scenario``: test scenarios, applicable to ``estimator`` or ``object_instance``.
...
applied to an ``object_instance``, via instances ``scenario``.
A call ``result = scenario.run(object_instance)`` will:

1. first, call ``object_instance.fit(y=_make_series(n_timepoints=20, random_state=RAND_SEED))``
2. then, call ``object_instance.predict(fh=1)`` and return the  output too ``result``.
...
For instance, the value of ``object_instance``, if this is a variable used in the test.
