...
class Pipeline(BaseEstimator):
    ...
    def __init__(self, steps):
        super().__init__()
        self.steps =  steps
        # Find all methods that are available for the last element in the pipeline.
        # Assume that the relevant methods starts either with predict or transform

        allowed_methods = list(filter(lambda x: x.startswith("predict") or x.startswith("transform"), dir(self.steps[-1])))

        for method in allowed_methods:
            setattr(self, method, functools.partial(self._generic_method, method))


    def _generic_method(self, function_name, **kwargs):
        ...
        for transformer in self.steps[:-1]:
            ...
            kwargs["X"] = X
        required_kwargs = self._check_validity(self.steps[-1], "_" + function_name, **kwargs)
        f = self.steps[-1]
        return getattr(f, function_name)(**required_kwargs)
...
if __name__ == "__main__":
    ...
    result = pipe.predict_proba(X=X_test)
    plt.plot(result)
    plt.show()