...
class AlignerDTW(BaseAligner):
    ...
    >>> X = [pd.DataFrame({'col1': np.random.randn(100)}), pd.DataFrame({'col1': np.random.randn(100)})]
    ...
    >>> aligner_advanced = AlignerDTW(dist_method='cityblock', window_type='sakoechiba', open_begin=True, open_end=True)
    >>> X_advanced = [pd.DataFrame({'col1': np.random.randn(150)}), pd.DataFrame({'col1': np.random.randn(150)})]
    >>> aligner_advanced.fit(X_advanced)
    >>> alignment_df_advanced = aligner_advanced.get_alignment()
    >>> print(alignment_df_advanced)
    """
...
class AlignerDTWfromDist(BaseAligner):
    ...
    >>> aligner_custom = AlignerDTWfromDist(dist_trafo=dist_trafo_custom, window_type='sakoechiba', open_begin=True)
