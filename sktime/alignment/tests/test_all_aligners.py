...
class TestAllAligners(AlignerFixtureGenerator, QuickTester):
    ...
    def test_get_alignment(self, object_instance):
        ...
        if object_instance.get_tag("alignment_type") == "full":
        ...
        if object_instance.get_tag("capability:multiple-alignment"):
            Xm = [_make_series(n_columns=2) for _ in range(3)]
            alignm = object_instance.fit(Xm).get_alignment()
            check_raise(alignm, mtype="alignment", scitype="Alignment")

    def test_get_alignment_loc(self, object_instance):
        """Test that get_alignment returns an alignment (loc)."""
        X = [_make_series(n_columns=2), _make_series(n_columns=2)]
        align = object_instance.fit(X).get_alignment_loc()
    ...
    def test_get_aligned(self, object_instance):
        """Test that get_aligned returns aligned series."""
        X = [_make_series(n_columns=2), _make_series(n_columns=2)]
        n = len(X)
        X_aligned = object_instance.fit(X).get_aligned()

        cls_name = type(object_instance).__name__
    ...
    def test_get_distance(self, object_instance):
        """Test that get_distance returns an scalar."""
        if not object_instance.get_tag("capability:distance"):
            return None

        X = [_make_series(n_columns=2), _make_series(n_columns=2)]
        dist = object_instance.fit(X).get_distance()
        assert isinstance(dist, float)

    def test_get_distance_matrix(self, object_instance):
        """Test that get_distance_matrix returns an scalar."""
        if not object_instance.get_tag("capability:distance-matrix"):
            return None

        X = [_make_series(n_columns=2), _make_series(n_columns=2)]
        dist = object_instance.fit(X).get_distance_matrix()
