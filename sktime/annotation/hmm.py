...
class HMM(SimpleBaseEstimator):
    ...
    def __init__(
        ...
        inital_probs: list = None,
    ):
        if not inital_probs:
            inital_probs = [
                1.0 / (len(emission_funcs)) for i in range(len(emission_funcs))
            ]
        params = {
            "emission_funcs": emission_funcs,
            "transition_prob_mat": transition_prob_mat,
            "num_states": len(emission_funcs),
            "states": [i for i in range(len(emission_funcs))],
            "initial_probs": inital_probs,
    ...
    def _calculate_trans_mats(self):
        ...
        # state at that stage
        trans_prob = np.zeros([self.num_states, self.num_obs])
        trans_prob[:, 0] = np.log(self.initial_probs)

        # trans_id is the index of the state that would have been the most
        # likely preceeding state.
        trans_id = np.zeros([self.num_states, self.num_obs]).astype(int)
            ...
            raise ValueError(
                "".join(
                    [
                        ...
                        "change your emission function parameters, or try",
                        "again with a shorter distribution.",
                    ]
                )
            )
    ...
    def fit(self, observations: np.ndarray):
        ...
        return self._calculate_trans_mats()
