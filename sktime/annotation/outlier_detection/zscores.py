...
__author__ = ["yelenayu"]
__all__ = ["MovingAverageZscoreOutlier"]
...
class MovingAverageZscoreOutlier(BaseTransformer):
    """Detect outliers using the moving average and modified z-score method.

    This method utilizes a sliding window.
    ...
    Examples
    --------
    >>> from sktime.transformations.series.outlier_detection import (
    ...     MovingAverageZscoreOutlier,
    ...)
    ...
    def __init__(
        self, window_length=10, threshold=3.5, use_modified_z=True, return_bool=False
    ):
    ...
    def _transform_series(self, Z):
        """Transform the series Z by detecting outliers.

        This method use moving average and z-score or modified z-score.
        ...
        if Z.isnull().values.any():
            warnings.warn(
                "Series contains nan values, more nan might be added for outliers",
                stacklevel=2,
            )
        ...
        for i in cv.split(Z):
            ...
            if cv_window[0] < self.window_length:
                # Edge case: first window - Use the data in this current window
