...
"""Time series data simulator for forecasting benchmarking.
...
class TimeSeriesSimulator(BaseEstimator):
    ...
    _time_index : pd.DatetimeIndex
        Generated time index for the series.
    _random_state : np.random.RandomState
        Random state instance used for generation.
    ...
    def simulate(self):
        ...
        if self.noise_std > 0:
            ...
            values = values + noise
        y = pd.Series(values, index=self._time_index, name="value")
