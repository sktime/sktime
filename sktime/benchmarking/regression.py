...
class RegressionBenchmark(BaseBenchmark):
    ...
    Run a series of regressors against a series of tasks defined via dataset loaders,
    ...
    def add_task(
        ...
        """Register a regression task to the benchmark.
        ...
        scorers : a list of callable objects
            Each scorer output will be included in the results.
            Scorers should accept ``(y_true, y_pred)`` and return a float.
        ...
        Returns
        -------
        A dictionary of benchmark results for that regressor
        """
        if task_id is None:
    ...
    def _run_validation(self, task: TaskObject, estimator: BaseRegressor):
        cv_splitter = task.cv_splitter
        scorers = task.scorers
        xy_dict = task.get_y_X("regression")
        scores_df = evaluate(
            regressor=estimator,
            cv=cv_splitter,
            scoring=scorers,
