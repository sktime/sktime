"""Residual Network (ResNet) Classifier in PyTorch."""
...
class ResNetClassifierTorch(BaseDeepClassifierPytorch):
    """Residual Neural Network Classifier in PyTorch, as described in [1]_.
    ...
    >>> clf = ResNetClassifierTorch(n_epochs=20)     # doctest: +SKIP
    ...
    def _build_network(self, X, y):
        """Build the ResNet network for classification.
        ...
        if len(np.unique(y)) == 1:
            warnings.warn(
                "The provided data contains a single label. "
                "If this is not intentional, please check your data.",
                UserWarning,
            )
