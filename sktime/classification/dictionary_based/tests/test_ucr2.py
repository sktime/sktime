...
def get_classifiers(threads_to_use):
    ...
    clfs = {
        ...
        "BOSS (new, boss-distance)": BOSSEnsemble(
            random_state=1379, use_boss_distance=True, n_jobs=threads_to_use
        ),
        "BOSS (new, ED)": BOSSEnsemble(
            random_state=1379, n_jobs=threads_to_use, use_boss_distance=False
        ),
        "cBOSS (new, boss-distance)": ContractableBOSS(
            random_state=1379, n_jobs=threads_to_use
        ),
        # "cBOSS (final)": ContractableBOSS(
        #     random_state=1379,
        #     n_jobs=threads_to_use
        # ),
...
if __name__ == "__main__":
    ...
    def _parallel_fit(dataset_name, clf_name):
        ...
        try:
            ...
            # print(e)
            # sum_scores[clf_name]["dataset"].append(dataset_name)
            # sum_scores[clf_name]["all_scores"].append(0)
            # sum_scores[clf_name]["all_fit"].append(0)
            # sum_scores[clf_name]["all_pred"].append(0)
    ...
    if server:
        ...
        ).to_csv("classifier_all_scores_test-06-09-22-final.csv", index=None)
        ...
        ).to_csv("classifier_all_runtimes_test-06-09-22-final.csv", index=None)
