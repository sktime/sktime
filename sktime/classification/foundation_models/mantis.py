...
default="full"
    ...
    def _build_trainer(self):
        ...
        if self.fine_tuning_type in ("full", "scratch"):
            ...
            buf.seek(0)
            network_copy.load_state_dict(torch.load(buf, weights_only=True, map_location=self._device_))
    ...
    def _ensure_trainer_loaded(self):
        ...
        if not hasattr(self, "_trainer_") or self._trainer_ is None:
            ...
            if network_bytes is not None:
                state = torch.load(io.BytesIO(network_bytes), weights_only=True, map_location=self._device_)
            ...
            if model_bytes is not None and hasattr(self, "_label_encoder_"):
                ...
                state = torch.load(io.BytesIO(model_bytes), weights_only=True, map_location=self._device_)
