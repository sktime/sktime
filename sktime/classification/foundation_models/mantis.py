...
default="full"
    ...
    def _build_trainer(self):
        ...
        if self.fine_tuning_type in ("full", "scratch"):
            ...
            buf.seek(0)
            network_copy.load_state_dict(torch.load(buf, map_location="cpu", weights_only=True))
    ...
    def _ensure_trainer_loaded(self):
        ...
        if not hasattr(self, "_trainer_") or self._trainer_ is None:
            ...
            if network_bytes is not None:
                state = torch.load(io.BytesIO(network_bytes), map_location="cpu", weights_only=True)
