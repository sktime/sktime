...
def test_mrseql_on_gunpoint():
    ...
    # the multi-domain classifier (ss_clf) should produce as many features as the others (sax_clf and sfa_clf) combine
    np.testing.assert_equal(ss_clf.clf.coef_.shape[1], sfa_clf.clf.coef_.shape[1] + sax_clf.clf.coef_.shape[1])
