...
class TestAllClassifiers(ClassifierFixtureGenerator, QuickTester):
    ...
    def test_multivariate_input_exception(self, object_instance):
        ...
        if object_instance.get_tag("capability:multivariate"):
            return None
        ...
        if object_instance.is_composite():
    ...
    def test_classifier_output(self, object_instance, scenario):
        ...
        # run fit and predict
        y_pred = scenario.run(object_instance, method_sequence=["fit", "predict"])
        ...
        y_proba = scenario.run(object_instance, method_sequence=["predict_proba"])
        assert isinstance(y_proba, np.ndarray)
        assert y_proba.shape == (X_new_instances, n_classes)
        np.testing.assert_almost_equal(y_proba.sum(axis=1), 1, decimal=4)

        if object_instance.get_tag("capability:train_estimate"):
            if not hasattr(object_instance, "_get_train_probs"):
                raise ValueError(
            ...
            train_proba = object_instance._get_train_probs(X_train, y_train)
    ...
    def test_classifier_on_unit_test_data(self, object_class):
        """Test classifier on unit test data."""
        # we only use the first estimator instance for testing
        classname = object_class.__name__
        ...
        if "random_state" in object_instance.get_params().keys():
            object_instance.set_params(random_state=0)
        ...
        y_pred = object_instance.predict(X_test.iloc[indices])
        assert y_pred.dtype == y_train.dtype
        assert set(y_train).issuperset(set(y_pred))

        y_proba = object_instance.predict_proba(X_test.iloc[indices])
    ...
    def test_classifier_on_basic_motions(self, object_class):
        ...
        # numba dependent estimators in results_comparison
        # if that is the case, we skip the test
        if not _check_estimator_deps(object_class, severity="none"):
            return None
        ...
        if object_instance.is_composite():
            ...
            for sub_estimator in object_instance.get_params(deep=True).values():
        ...
        # train classifier and predict probas
        object_instance.fit(X_train.iloc[indices], y_train[indices])
        y_proba = object_instance.predict_proba(X_test.iloc[indices])

        # assert probabilities are the same
        _assert_array_almost_equal(y_proba, expected_probas, decimal=2)

    def test_handles_single_class(self, object_instance):
        ...
        with pytest.warns(UserWarning, match=error_msg):
            object_instance.fit(X, y)

    def test_multioutput(self, object_instance):
        ...
        object_instance.fit(X, y_mult)
        y_pred = object_instance.predict(X)
        ...
        if vectorized:
            assert hasattr(object_instance, "classifiers_")
            assert isinstance(object_instance.classifiers_, pd.DataFrame)
            assert object_instance.classifiers_.shape == (1, 2)

    def test_class_weight(self, object_instance, scenario):
        """Test that classifiers with class_weight handle it correctly."""
        # Check if the estimator supports class_weight
        if not object_instance.get_tag("capability:class_weight"):
            return None
        ...
        object_instance.set_params(class_weight=class_weight)

        # Fit and predict to check no error is raised
        object_instance.fit(X_train, y_train)
        y_pred = object_instance.predict(X_train)
