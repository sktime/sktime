...
class BarycenterAveraging(BaseClusterAverage):
    ...
    def __init__(self, series: Numpy_Array, n_iterations: int = 10):
        """
        Constructor for barycenter averaging
    ...
    def average(self):
        ...
        medoid_ind = self.__approximate_medoid_index(cost_mat, delta_mat)
        center = self.series[medoid_ind]

        for _ in range(self.n_iterations):
            center = self.__DBA_update(center, cost_mat, path_mat, delta_mat)

        return center

    def __approximate_medoid_index(self, cost_mat, delta_mat):
        ...
        for index_candidate in indices:
            ...
            ss = self.__sum_of_squares(candidate, cost_mat, delta_mat)
            if medoid_ind == -1 or ss < best_ss:
                best_ss = ss
                medoid_ind = index_candidate
        return medoid_ind

    def __sum_of_squares(self, s, cost_mat, delta_mat):
        return sum(
            map(lambda t: self.__squared_DTW(s, t, cost_mat, delta_mat), self.series)
        )

    def __DTW(self, s, t, cost_mat, delta_mat):
        return np.sqrt(self.__squared_DTW(s, t, cost_mat, delta_mat))

    def __squared_DTW(self, s, t, cost_mat, delta_mat):
        s_len = len(s)
        t_len = len(t)
        self.__fill_delta_mat_dtw(s, t, delta_mat)
    ...
    def __fill_delta_mat_dtw(self, center, s, delta_mat):
        slim = delta_mat[: len(center), : len(s)]
        np.subtract.outer(center, s, out=slim)
        np.square(slim, out=slim)

    def __DBA_update(self, center, cost_mat, path_mat, delta_mat):
        ...
        for s in self.series:
            s_len = len(s)
            self.__fill_delta_mat_dtw(center, s, delta_mat)
            cost_mat[0, 0] = delta_mat[0, 0]
            path_mat[0, 0] = -1
