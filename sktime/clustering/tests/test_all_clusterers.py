...
class TestAllClusterers(ClustererFixtureGenerator, QuickTester):
    ...
    def test_clusterer_output(self, object_instance, scenario):
        ...
        if not object_instance.get_tag("capability:predict"):
            return None

        # run predict
        y_pred = scenario.run(object_instance, method_sequence=["predict"])
        ...
        if hasattr(object_instance, "n_clusters") and object_instance.n_clusters:
            assert y_proba.shape[1] == object_instance.n_clusters
