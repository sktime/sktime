...
def _reproduce_unit_test(estimator):
    X_train, y_train = load_unit_test(split="train", return_X_y=True)
    X_test, y_test = load_unit_test(split="test", return_X_y=True)
    indices = np.random.RandomState(0).choice(len(y_train), 10, replace=False)

    estimator.fit(X_train, y_train)
    probas = estimator.predict_proba(X_test.iloc[indices])

    return probas


def _reproduce_basic_motions(estimator):
...
if __name__ == "__main__":
    _print_array(
        ...
        _reproduce_unit_test(BOSSEnsemble(max_ensemble_size=5, random_state=0)),
    )
    _print_array(
        "IndividualBOSS - UnitTest",
        _reproduce_unit_test(IndividualBOSS(random_state=0)),
    )
    _print_array(
        "ContractableBOSS - UnitTest",
        _reproduce_unit_test(
            ContractableBOSS(
    ...
    _print_array(
        "WEASEL - UnitTest", _reproduce_unit_test(WEASEL(random_state=0, window_inc=4))
    )
    _print_array(
    ...
    _print_array(
        ...
        _reproduce_unit_test(ProximityForest(n_estimators=5, random_state=0)),
    )
    _print_array("ShapeDTW - UnitTest", _reproduce_unit_test(ShapeDTW()))
    _print_array(
    ...
    _print_array(
        ...
        _reproduce_unit_test(MatrixProfileClassifier(random_state=0)),
    )
    _print_array(
    ...
    _print_array(
        ...
        _reproduce_unit_test(CanonicalIntervalForest(n_estimators=10, random_state=0)),
    )
    _print_array(
    ...
    _print_array(
        "DrCIF - UnitTest", _reproduce_unit_test(DrCIF(n_estimators=10, random_state=0))
    )
    _print_array(
        "DrCIF - BasicMotions",
        _reproduce_basic_motions(DrCIF(n_estimators=10, random_state=0)),
    )
    _print_array(
    ...
    _print_array(
        "Arsenal - UnitTest",
        _reproduce_unit_test(Arsenal(num_kernels=200, n_estimators=5, random_state=0)),
    )
    _print_array(
    ...
    _print_array(
        ...
        _reproduce_unit_test(RocketClassifier(num_kernels=500, random_state=0)),
    )
    _print_array(
        "RocketClassifier - BasicMotions",
        _reproduce_basic_motions(RocketClassifier(num_kernels=500, random_state=0)),
    )
    _print_array(
