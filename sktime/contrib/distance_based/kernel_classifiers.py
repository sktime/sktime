...
def build_eig_zero_svm(
    ...
    pipe = sklearn.pipeline.Pipeline([
            ('pd_to_np', PandasToNumpy()),
            ('d', EigKernel(transformer = NegToZero())),
            ('svm', SVC(probability = True, kernel = 'precomputed')),
            ])
    cv_params = {
            'svm__C': scipy.stats.expon(scale = 100)
...
def build_eig_abs_svm(
    ...
    pipe = sklearn.pipeline.Pipeline([
            ('pd_to_np', PandasToNumpy()),
            ('d', EigKernel(transformer = NegToAbs())),
            ('svm', SVC(probability = True, kernel = 'precomputed')),
            ])
    cv_params = {
            'svm__C': scipy.stats.expon(scale = 100)
...
def build_eig_min_svm(
    ...
    pipe = sklearn.pipeline.Pipeline([
            ('pd_to_np', PandasToNumpy()),
            ('d', EigKernel(transformer = NegToMin())),
            ('svm', SVC(probability = True, kernel = 'precomputed')),
            ])
    cv_params = {
            'svm__C': scipy.stats.expon(scale = 100)
