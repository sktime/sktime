...
class RotationForest(ForestClassifier):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self._noise = [
            random.uniform(-0.000005, 0.000005) for p in range(0, X.shape[1])
        ]  # in case we have 0 std.
        ...
        for i in range(0, self.n_estimators):
            ...
            for j, grp in enumerate(self._groups[i]):
                ...
                sample_ind = random.choice(
                    len(rot_j),
                    int((float(len(rot_j)) / 100.0) * self.remove_percentage),
                    replace=False,
                )
    ...
    def createRow(self, row, i):
        return np.concatenate(
            [
                self._pcas[i][j].transform([row[grp]])[0]
                for j, grp in enumerate(self._groups[i])
            ],
            axis=0,
        )
    ...
    def createClassesGrouping(self, X, Y):
        return np.array(
            [
                np.squeeze(X[np.argwhere(Y == i)]).reshape((-1, self._num_atts))
