...
class RotationForest(ForestClassifier):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for i in range(0, self.n_estimators):
            ...
            for j, grp in enumerate(self._groups[i]):
                ...
                sample_ind = random.choice(len(rot_j), int((float(len(rot_j)) / 100.0) * self.remove_percentage),
                                           replace=False)
    ...
    def createRow(self, row, i):
        return np.concatenate([self._pcas[i][j].transform([row[grp]])[0] for j, grp in enumerate(self._groups[i])],
                              axis=0)
