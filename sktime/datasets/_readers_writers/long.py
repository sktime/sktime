...
def load_from_long_to_dataframe(full_file_path_and_name, separator=","):
    ...
    # ensure that all columns contain compatible data types
    # first three columns are identifiers/indices and must be integer-like;
    # fourth column holds observed values and must be numeric.
    if (
        not is_integer_dtype(data.iloc[:, 0].dtype)
        or not is_integer_dtype(data.iloc[:, 1].dtype)
        or not is_integer_dtype(data.iloc[:, 2].dtype)
        or not is_numeric_dtype(data.iloc[:, 3].dtype)
    ):
