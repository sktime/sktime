...
def load_stallion(special_dates=True, verbose=False):
    ...
    """
    name = "Stallion"
    path = os.path.join(MODULE, DIRNAME, name, name + ".csv.zip")
    path_extra = os.path.join(MODULE, DIRNAME, name, name + "_extra.csv.zip")
    ...
    if special_dates:
        data_extra = pd.read_csv(path_extra)
        data_extra["date"] = pd.to_datetime(data_extra["date"])
        data_extra = data_extra.set_index(["agency", "sku", "date"])
        x = x.join(data_extra)
    return y, x
