...
def load_gunpoint(split='TRAIN', return_X_y=True):
    # I've left the arg in to not break other code before the demo but we shouldn't
    # allow for the y to be embedded in the X!
    if return_X_y is not True:
        raise ValueError("sktime datasets should have separate X and y")
    ...
    return load_from_tsfile_to_dataframe(abspath)


def load_italy_power_demand(split='TRAIN'):
...
def load_arrow_head(split='TRAIN'):
