...
class TestAllDatasets(DatasetFixtureGenerator, QuickTester):
    ...
    def test_tag_n_instances(self, object_instance):
        """Check the number of instances."""
        n_instances = object_instance.get_tag("n_instances")
        y = object_instance.load("y")
    ...
    def test_tag_n_instances_test(self, object_instance):
        """Check the number of instances in the test set.


        If the dataset has no training set, the tests for InvalidSetError
        """
        n_instances_test = object_instance.get_tag("n_instances_test")

        contextwrapper = (
            nullcontext() if n_instances_test > 0 else pytest.raises(InvalidSetError)
        )
        with contextwrapper:
            y_test = object_instance.load("y_test")
    ...
    def test_tag_n_splits(self, object_instance):
        """Check the number of splits."""
        n_splits = object_instance.get_tag("n_splits")
        if n_splits > 1:
            cv = object_instance.load("cv")
            # assert length of cv is equal to the number of splits
            assert len(list(cv)) == n_splits
        else:
            with pytest.raises(InvalidSetError):
                object_instance.load("cv")
