...
def test_pandas_to_polars_with_pandas_period_index():
    ...
    result = convert_pandas_to_polars(df)

    assert result.columns == ["__index__0", "a", "b", "foo", "bar"]
...
def test_pd_multiindex_to_polars_with_pandas_period_index():
    ...
    index = pd.MultiIndex.from_arrays(arrays, names=["level_1", "level_2", "period"])
    pd_multiindex_period_index = pd.DataFrame(
        data, index=index, columns=["foo", "bar", "a"]
    )

    result = convert_pandas_to_polars(pd_multiindex_period_index)
