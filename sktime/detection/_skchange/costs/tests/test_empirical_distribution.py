...
from sktime.detection._skchange.utils.numba.general import (
...
def test_fixed_cdf_empirical_distribution_cost_vs_direct_cost():
    ...
    np.testing.assert_allclose(direct_cost, fixed_cdf_cost, rtol=1e-12, atol=1e-12)
    np.testing.assert_allclose(
        direct_cost, fixed_cdf_cost_cached, rtol=1e-12, atol=1e-12
    )
...
def test_min_size_is_num_approximation_quantiles():
    ...
    assert cost.min_size == num_approximation_quantiles, (
        "Expected min_size to be "
        f"{num_approximation_quantiles}, but got {cost.min_size}"
    )
