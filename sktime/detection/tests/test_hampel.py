...
@pytest.mark.skipif(
    not run_test_for_class(HampelDetector),
...
def test_predict_basic(X, y_expected, window_size, n_sigmas):
    ...
    model = HampelDetector(window_size=window_size, n_sigmas=n_sigmas)
    model.fit(X)
...
def test_hampel_does_not_mutate_input():
    """Check that HampelDetector does not modify the input DataFrame."""
    X = pd.DataFrame([2, 5, 7, 9, 12, 55, 1, 3, 2, 1])
    X_original = X.copy(deep=True)

    model = HampelDetector(window_size=5, n_sigmas=3.0)
...
def test_short_series():
    ...
    model = HampelDetector(window_size=5)
    model.fit(X)
    y = model.predict(X)
...
def test_even_window_size_adjustment():
    ...
    model_centered = HampelDetector(window_size=4, center=True)
    model_trailing = HampelDetector(window_size=4, center=False)
...
def test_center_parameter():
    ...
    model_centered = HampelDetector(window_size=5, center=True, n_sigmas=3.0)
    model_trailing = HampelDetector(window_size=5, center=False, n_sigmas=3.0)
...
def test_fit_predict_vs_fit_transform():
    ...
    X = pd.DataFrame([2, 5, 7, 9, 12, 55, 1, 3, 2, 1])
    model1 = HampelDetector(window_size=5, n_sigmas=3.0)
    model2 = HampelDetector(window_size=5, n_sigmas=3.0)
...
def test_mmad_vs_standard():
    ...
    model_standard = HampelDetector(window_size=5, n_sigmas=3.0, use_mmad=False)
    model_mmad = HampelDetector(window_size=5, n_sigmas=3.0, use_mmad=True)
...
def test_various_window_sizes(window_size):
    ...
    model = HampelDetector(window_size=window_size, n_sigmas=3.0)
    model.fit(X)
    y = model.predict(X)
...
def test_threshold_sensitivity():
    ...
    model_strict = HampelDetector(window_size=5, n_sigmas=2.0)
    model_lenient = HampelDetector(window_size=5, n_sigmas=4.0)
...
def test_get_test_params():
    ...
    params = HampelDetector.get_test_params()
    ...
    for param_dict in params:
        ...
        model = HampelDetector(**param_dict)
...
def test_invalid_mmad_window():
    ...
    with pytest.raises(ValueError, match="mmad_window must be >= 3"):
        HampelDetector(use_mmad=True, mmad_window=2)
...
def test_mmad_window_without_use_mmad():
    ...
    with pytest.raises(ValueError, match="mmad_window requires use_mmad=True"):
        HampelDetector(use_mmad=False, mmad_window=5)
