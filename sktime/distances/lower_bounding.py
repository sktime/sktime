...
def sakoe_chiba(x: np.ndarray, y: np.ndarray, window: float) -> np.ndarray:
    ...
    bounding_matrix = np.full((x.shape[1], y.shape[1]), np.inf)
    sakoe_chiba_window_radius = ((x.shape[1] / 100) * window) * 100
...
def itakura_parallelogram(
    ...
    if itakura_max_slope < 0 or itakura_max_slope > 1:
        raise ValueError("Window must between 0 and 1")
    bounding_matrix = np.full((y.shape[1], x.shape[1]), np.inf)
    itakura_max_slope = math.floor(((x.shape[1] / 100) * itakura_max_slope) * 100) / 2
