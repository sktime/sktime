...
def _edr_cost_matrix(
    ...
    epsilon : float, defaults = 0.5
        Matching threshold to determine if two series are considered 'close'.
    """
    x_size = x.shape[0]
    y_size = y.shape[0]
...
def _numba_edr_distance(
    ...
    """Method that is a numba compiled version of edr distance.
    ...
    return float(cost_matrix[-1, -1] / max(_x.shape[0], _y.shape[0]))
...
def edr_distance(
    ...
    """Method to calculate the edr distance between timeseries.
    ...
    -------
    float
        edr distance between the two timeseries
    """
...
def numba_edr_distance_factory(
    x: np.ndarray,
    y: np.ndarray,
    ...
    """Method used to produce a numba edr function.
    ...
    -------
    Callable[[np.ndarray, np.ndarray], float]
        Method that calculates the edr distance between two timeseries
    """
    bounding_matrix = _resolve_bounding_matrix(
    ...
    @njit()
    def numba_edr(_x: np.ndarray, _y: np.ndarray) -> float:
        return _numba_edr_distance(
...
def pairwise_edr_distance(
    ...
    """Edr pairwise distance between two timeseries.
    ...
    -------
    np.ndarray
        Pairwise matrix calculated using edr
    """
    return pairwise_distance(
        x,
        y,
        numba_distance_factory=numba_edr_distance_factory,
