...
class TransferEntropy(BasePairwiseTransformerPanel):
    ...
    Transfer Entropy (TE) is an information-theoretic measure that quantifies directed
    ...
    Notes
    -----
    - Higher TE values indicate strong directed information flow from source to target.
    - TE = 0 implies conditional independence (target independent of source).
    ...
    def _transform(self, X, X2=None):
        ...
        te_matrix = pd.DataFrame(
            np.zeros((n_series, n_series)), index=col_names, columns=col_names
        )
    ...
    def _get_transfer_entropy(self, source, target):
        ...
        def create_lagged_vectors(series, lag):
            ...
            return (
                ...
                if vectors
                else np.empty((len(series) - lag + 1, 0))
            )
        ...
        y_target_hist = create_lagged_vectors(target, self.lag_target)[
            max(0, self.lag_source - self.lag_target) :, :
        ]  # y_(t-1:t-k)
        x_source_hist = create_lagged_vectors(source, self.lag_source)[
            max(0, self.lag_target - self.lag_source) :, :
        ]  # x_(t-1:t-l)
    ...
    def _calculate_te_binning(self, y_target, y_target_hist, x_source_hist):
        ...
        def discretize(data, n_bins):
            ...
            if data.ndim == 1:
                return np.digitize(
                    data, np.linspace(min(data), max(data), n_bins + 1)[:-1]
                )
        ...
        joint_counts_y = np.zeros(
            (self.n_bins, np.prod([self.n_bins] * y_hist_disc.shape[1]))
        )
        ...
        for y_hist_val in range(joint_counts_y.shape[1]):
            if np.sum(joint_counts_y[:, y_hist_val]) > 0:
                p_y_hist = np.sum(joint_counts_y[:, y_hist_val]) / np.sum(
                    joint_counts_y
                )
                h_y_given_y_hist_val = get_entropy_from_counts(
                    joint_counts_y[:, y_hist_val]
                )
        ...
        yx_hist_parts.extend(
            [x_hist_disc[:, i] - 1 for i in range(x_hist_disc.shape[1])]
        )

        yx_hist_idx = np.ravel_multi_index(yx_hist_parts, [self.n_bins] * joint_dim)
        ...
        for yx_hist_val in range(joint_counts_yx.shape[1]):
            if np.sum(joint_counts_yx[:, yx_hist_val]) > 0:
                p_yx_hist = np.sum(joint_counts_yx[:, yx_hist_val]) / np.sum(
                    joint_counts_yx
                )
                h_y_given_yx_hist_val = get_entropy_from_counts(
                    joint_counts_yx[:, yx_hist_val]
                )
    ...
    def _calculate_te_knn(self, y_target, y_target_hist, x_source_hist):
        ...
        eps = dists_y_yhist[:, k].reshape(
            -1, 1
        )  # use Chebyshev distance of k-th neighbor
        ...
        n_y = np.array(
            [
                np.sum(np.abs(y_target - y_target[i]) <= eps[i])
                for i in range(len(y_target))
            ]
        )
        n_yhist = np.array(
            [
                np.sum(np.all(np.abs(space_yhist - space_yhist[i]) <= eps[i], axis=1))
                for i in range(len(space_yhist))
            ]
        )
        n_yhist_xhist = np.array(
            [
                np.sum(
                    np.all(
                        np.abs(space_yhist_xhist - space_yhist_xhist[i]) <= eps[i],
                        axis=1,
                    )
                )
                for i in range(len(space_yhist_xhist))
            ]
        )

        # Apply digamma function and calculate transfer entropy
        te = digamma(k) + np.mean(
    ...
    def _calculate_te_kernel(self, y_target, y_target_hist, x_source_hist):
        ...
        joint_y_yhist_xhist = np.column_stack(
            [y_target.reshape(-1, 1), y_target_hist, x_source_hist]
        )
