...
class BaseFormerNetworkPyTorch(BaseForecaster, ABC):
    """Abstract base class for deep learning networks using torch.nn."""
    ...
    def _fit(self, y, fh, X=None):
        ...
        dataloader = self.build_pytorch_train_dataloader(y, X)
        self.network.train()
...
class PyTorchFormerTrainDataset(Dataset):
    """Dataset for use in sktime deep learning forecasters."""

    def __init__(self, y, seq_len, fh=None, X=None):
        self.y = y.values if y.values.ndim != 1 else y.values.reshape(-1, 1)
        self.X = X.values if X is not None else X
        self.seq_len = seq_len
        self.fh = fh
    ...
    def __getitem__(self, i):
        ...
        # x_dec         # (batch_size, pred_len, num_features)      target sequence for the decoder
        # x_mark_dec    # (batch_size, pred_len, num_time_features) time embeddings (or positional embeddings) of target sequence for decoder

        enc_start_i = i
        enc_end_i = enc_start_i + self.seq_len

        dec_start_i = i + self.seq_len
        dec_end_i = dec_start_i + self.fh

        x_enc = tensor(self.y[enc_start_i:enc_end_i]).float()
        x_mark_enc = tensor(self.X[enc_start_i:enc_end_i]).float()
        x_dec = tensor(self.y[dec_start_i:dec_end_i]).float()
