...
class DMLForecaster(BaseForecaster):
    ...
    def _split_exogenous_data(self, X):
        ...
        if missing_vars:
            raise ValueError(
                f"Exposure variables {missing_vars} not found in X columns: {list(X.columns)}"
            )
    ...
    def _fit(self, y, X=None, fh=None):
        ...
        X_ex_pred_insample = self.forecaster_ex_insample_.predict(fh=insample_fh, X=X_conf)
    ...
    def _predict_var(self, fh, X=None, cov=False):
        ...
        # Base variance
        var_conf = self.forecaster_y_final_.predict_var(
            fh=fh, X=X_conf_new, cov=cov
        )
        ...
        # Causal variance
        var_causal = self.forecaster_res_.predict_var(
            fh=fh, X=X_ex_deconf, cov=cov
        )
    ...
    def summary(self):
        """Provide a summary of the fitted DML forecaster.
        
        Returns
        -------
        summary_str : str
            Summary of the DML forecaster including causal effect estimates.
        """
        if not self.is_fitted:
            return "DMLForecaster not fitted yet."
            
        summary_parts = [
            "=== Double Machine Learning Forecaster Summary ===",
            f"Exposure variables: {self.exposure_vars}",
        ]
        
        # Add causal effect estimate if available
        try:
            if hasattr(self.forecaster_res_, 'estimator_'):
                estimator = self.forecaster_res_.estimator_
                if hasattr(estimator, 'coef_'):
                    ...
                    if len(coef.shape) == 1:
                        for i, var in enumerate(self.exposure_vars):
                            summary_parts.append(f"Causal effect of {var}: {coef[i]:.4f}")
                    else:
                        summary_parts.append(f"Causal effects: {coef}")
        except (AttributeError, IndexError):
            summary_parts.append("Causal effects: Not available (complex residual forecaster)")
            
        summary_parts.extend([
            "\nComponent Forecasters:",
            f"- Outcome forecaster: {type(self.forecaster_y).__name__}",
            f"- Exposure forecaster: {type(self.forecaster_ex).__name__}",
            f"- Residual forecaster: {type(self.forecaster_res).__name__}",
        ])
