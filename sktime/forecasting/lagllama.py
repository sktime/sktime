...
class LagLlamaForecaster(BaseForecaster):
    ...
    **Fine-tuning with pretrain() on panel data**
    ...
    def _predict_quantiles(self, fh, X=None, alpha=None):
        ...
        private _predict_quantiles containing the core logic, called from predict_quantiles

        The Monte Carlo sample paths generated by LagLlama are used to compute
        quantiles directly via numpy, so this method works without skpro.
        ...
        # Use the shared forecast generation
        forecasts, is_hierarchical, original_index_names = self._generate_forecasts(fh)

        # Assemble samples in wide format for quantile computation
        sample_dfs = []
        ...
        # Compute quantiles directly from samples using numpy
        quantile_vals = np.percentile(wide.values, [a * 100 for a in alpha], axis=1)

        # Build quantile DataFrame in sktime format
        var_name = self._get_varname()
        if isinstance(wide.index, pd.MultiIndex):
            # Panel/hierarchical: columns = MultiIndex(variable, alpha), index = (item, time)
            quantile_dfs = []
            for i, a in enumerate(alpha):
                ...
                q_df.columns = pd.MultiIndex.from_tuples(
                    [(var_name, a)], names=["variable", "alpha"]
                )
                quantile_dfs.append(q_df)
            result = pd.concat(quantile_dfs, axis=1)
        else:
            # Single series: columns = MultiIndex(variable, alpha), index = time
            result = pd.DataFrame(
                quantile_vals.T,
                index=wide.index,
                columns=pd.MultiIndex.from_tuples(
                    [(var_name, a) for a in alpha], names=["variable", "alpha"]
                ),
            )
