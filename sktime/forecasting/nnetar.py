...
class NNetAR(BaseForecaster):
    """Forecast with an ensemble of neural autoregression models.

    NNetAR turns a univariate time series into a one-step-ahead regression
    problem. For each time point ``t``, the model uses the non-seasonal values
    ``y[t - 1], ..., y[t - p]`` and seasonal values
    ``y[t - sp], ..., y[t - P * sp]`` as features, with ``y[t]`` as the target.
    A feed-forward ``MLPRegressor`` with one hidden layer is fitted to these
    lagged feature-target pairs.

    ``repeats`` networks are fitted from different random initial weights, and
    their one-step-ahead predictions are averaged. Multi-step forecasts are
    generated recursively: each averaged prediction is appended to the history
    and becomes an input lag for subsequent forecast steps.
