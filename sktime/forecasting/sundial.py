...
class SundialForecaster(BaseForecaster):
    ...
    num_samples : int, default=20
        Number of sample paths generated by Sundial. Point forecasts average
        over this sample axis; quantile forecasts use empirical quantiles.
    forward_kwargs : dict, optional (default=None)
        Additional keyword arguments forwarded to ``model.generate(...)`` during
        prediction, for example ``{"revin": True}``.
    ...
    >>> from sktime.forecasting.sundial import SundialForecaster
    >>> y = load_airline()
    >>> forecaster = SundialForecaster(num_samples=50)  # doctest: +SKIP
    >>> forecaster.fit(y)  # doctest: +SKIP
