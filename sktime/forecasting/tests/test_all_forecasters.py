...
class TestAllForecasters(ForecasterFixtureGenerator, QuickTester):
    ...
    def test_get_fitted_params(self, object_instance, scenario):
        """Test get_fitted_params."""
        scenario.run(object_instance, method_sequence=["fit"])

        params = object_instance.get_fitted_params()
    ...
    def test_categorical_X_passes(self, estimator_instance):
        ...
        ) and estimator_instance.get_tag("capability:categorical_in_X"):
            y_train = _make_series(n_timepoints=6, n_columns=2)
            X_train = pd.DataFrame({"col_0": ["a", "b", "c", "a", "b", "c"]}, index=y_train.index)
    ...
    def test_categorical_y_raises_error(self, object_instance):
    ...
    def test_predict_time_index(
        self, object_instance, n_columns, index_fh_comb, fh_int
    ):
        ...
        if not fh_is_oos and not object_instance.get_tag("capability:insample"):
            with pytest.raises(NotImplementedError, match="in-sample"):
                object_instance.fit(y_train, fh=fh)
            return None
        ...
        y_res = object_instance.predict_residuals(y_test)
        ...
        # check that estimator raises error message when fitting
        if not fh_is_oos:
            if not object_instance.get_tag("capability:pred_int:insample"):
                if object_instance.get_tag("capability:pred_int"):
                    with pytest.raises(
                        NotImplementedError,
                        match="can not perform in-sample prediction",
                    ):
                        object_instance.predict_interval()
                    return None

        if object_instance.get_tag("capability:pred_int"):
            y_pred_int = object_instance.predict_interval()
            _assert_correct_pred_time_index(y_pred_int.index, cutoff, fh=fh_int)
            y_pred_q = object_instance.predict_quantiles()
    ...
    def test_predict_time_index_with_X(
        self, object_instance, n_columns, index_fh_comb, fh_int_oos
    ):
        """Check that predicted time index matches forecasting horizon."""
        index_type, fh_type, is_relative = index_fh_comb
        if fh_type == "timedelta":
        ...
        if not fh._is_contiguous() and not object_instance.get_tag(
        ...
        object_instance.fit(y_train, X_train, fh=fh)
        ...
        if object_instance.get_tag("capability:pred_int"):
            y_pred_int = object_instance.predict_interval(X=X_test)
            _assert_correct_pred_time_index(y_pred_int.index, cutoff, fh)
            y_pred_q = object_instance.predict_quantiles(X=X_test)
            ...
            with pytest.raises(NotImplementedError, match="prediction intervals"):
                object_instance.predict_interval(X=X_test)

    @pytest.mark.parametrize(
        "index_fh_comb", VALID_INDEX_FH_COMBINATIONS, ids=index_fh_comb_names
    )
    def test_predict_time_index_in_sample_full(
        self, object_instance, n_columns, index_fh_comb
    ):
        ...
        can_pr_int = object_instance.get_tag("capability:pred_int")
        can_pr_iins = object_instance.get_tag("capability:pred_int:insample")
        can_pr_iins = can_pr_int and can_pr_iins
        can_pr_iins = can_pr_iins and object_instance.get_tag("capability:insample")
        if can_pr_iins:
            object_instance.fit(y_train, fh=fh)
            y_pred = object_instance.predict()
            _assert_correct_pred_time_index(y_pred.index, cutoff, fh)
    ...
    def test_predict_series_name_preserved(self, object_instance):
    ...
    def test_predict_interval(
        self, object_instance, n_columns, index_type, fh_int_oos, coverage
    ):
        """Check prediction intervals returned by predict_interval.

        Arguments
        ---------
        object_instance : BaseEstimator class descendant instance, forecaster to test
        ...
        """
        y_train = _make_series(n_columns=n_columns, index_type=index_type)
        object_instance.fit(y_train, fh=fh_int_oos)
        if object_instance.get_tag("capability:pred_int"):
            pred_ints = object_instance.predict_interval(
            ...
            with pytest.raises(NotImplementedError, match="prediction intervals"):
                object_instance.predict_interval(fh_int_oos, coverage=coverage)
    ...
    def test_predict_quantiles(self, object_instance, n_columns, fh_int_oos, alpha):
        ...
        object_instance.fit(y_train, fh=fh_int_oos)
        if object_instance.get_tag("capability:pred_int"):
            quantiles = object_instance.predict_quantiles(fh=fh_int_oos, alpha=alpha)
            ...
            with pytest.raises(NotImplementedError, match="quantile predictions"):
                object_instance.predict_quantiles(fh=fh_int_oos, alpha=alpha)

    @pytest.mark.parametrize(
        "fh_int_oos", TEST_OOS_FHS, ids=[f"fh={fh}" for fh in TEST_OOS_FHS]
    )
    def test_predict_proba(self, object_instance, n_columns, fh_int_oos):
        ...
        if object_instance.get_tag("capability:pred_int"):
            try:
                pred_dist = object_instance.predict_proba()
            ...
            with pytest.raises(NotImplementedError, match="probabilistic predictions"):
                object_instance.predict_proba()

    def test_pred_int_tag(self, object_instance):
    ...
    def test_score(self, object_instance, n_columns, fh_int_oos):
        """Check score method."""
        ...
        actual = object_instance.score(y_test.iloc[fh_idx], fh=fh_int_oos)
    ...
    def test_update_predict_single(
        self, object_instance, n_columns, fh_int_oos, update_params
    ):
        """Check correct time index of update-predict."""
        y = _make_series(n_columns=n_columns)
        y_train, y_test = temporal_train_test_split(y)
        object_instance.fit(y_train, fh=fh_int_oos)
        y_pred = object_instance.update_predict_single(
    ...
    def test_update_predict_predicted_index(
        ...
        object_instance.fit(y_train, fh=fh_int_oos)
        y_pred = object_instance.update_predict(
    ...
    def test_update_with_exogenous_variables(
        ...
        object_instance.fit(y_train, X=X_train, fh=FH0)

        # Test update with exogenous variables
        object_instance.update(y_test, X=X_test, update_params=update_params)
    ...
    def test_fh_not_passed_error_handling(self, object_instance, n_columns):
    ...
    def test_different_fh_in_fit_and_predict_error_handling(
        self, object_instance, n_columns
    ):
    ...
    def test_hierarchical_with_exogenous(self, object_instance, n_columns):
    ...
    def test_fit_predict(self, object_instance, n_columns):
    ...
    def test_unequal_length_hierarchical_fit_predict(self, object_instance):
    ...
    @staticmethod
    def _pretrain_fh(object_instance):
        ...
        """
        pred_len = getattr(object_instance, "pred_len", None) or 3
        return ForecastingHorizon(list(range(1, pred_len + 1)))

    def test_pretrain_capability_tag(self, object_instance):
        ...
        has_custom_pretrain = (
            object_instance.__class__._pretrain != BaseForecaster._pretrain
        )

        tag_value = object_instance.get_tag(
            "capability:pretrain", tag_value_default=False, raise_error=False
        )

        if has_custom_pretrain:
            assert tag_value is True, (
                f"{object_instance.__class__.__name__} implements _pretrain "
        ...
        if tag_value is True:
            assert has_custom_pretrain, (
                f"{object_instance.__class__.__name__} sets "
                f"capability:pretrain=True but does not override _pretrain method"
            )

    def test_pretrain_state_transitions(self, object_instance, n_columns):
        ...
        """
        if not object_instance.get_tag(
            "capability:pretrain", tag_value_default=False, raise_error=False
        ):
        ...
        assert object_instance.state == "new", (
            f"Initial state should be 'new', got {object_instance.state}"
        )

        fh = self._pretrain_fh(object_instance)
        ...
        object_instance.pretrain(y_panel2)
        assert object_instance.state == "pretrained", (
        ...
        object_instance.fit(y_series, fh=fh)
        assert object_instance.state == "fitted", (
            f"State after fit should be 'fitted', got {object_instance.state}"
        )

    def test_pretrain_fit_predict_workflow(self, object_instance, n_columns):
    ...
    def test_pretrain_not_reset_by_fit(self, object_instance, n_columns):
        ...
        pretrained_params_before = object_instance.get_pretrained_params()
        pretrained_attrs_before = list(pretrained_params_before.keys())

        y_series = _make_series(n_columns=n_columns)
        object_instance.fit(y_series, fh=fh)

        pretrained_params_after = object_instance.get_pretrained_params()
    ...
    def test_pretrain_rejects_single_series(self, object_instance, n_columns):
        ...
        with pytest.raises(TypeError, match="requires Panel or Hierarchical data"):
            object_instance.pretrain(y_single)

    def test_pretrain_with_hierarchical_data(self, object_instance, n_columns):
        ...
        """
        has_pretrain = object_instance.get_tag(
            "capability:pretrain", tag_value_default=False, raise_error=False
        )
        ...
        fh = self._pretrain_fh(object_instance)
        object_instance.pretrain(y_hier, fh=fh)
        assert object_instance.state == "pretrained"

        pretrained_params = object_instance.get_pretrained_params()
        assert len(pretrained_params) > 0, "Expected pretrained attributes to be set"

    def test_pretrain_predict_without_fit(self, object_instance, n_columns):
        ...
        with pytest.raises(NotFittedError):
            object_instance.predict(fh=fh)

    def test_pretrain_fitted_params_separation(self, object_instance, n_columns):
        ...
        pretrained_keys = set(object_instance.get_pretrained_params().keys())
        assert len(pretrained_keys) > 0, "Expected pretrained params after pretrain()"

        y_series = _make_series(n_columns=n_columns)
        object_instance.fit(y_series, fh=fh)

        fitted_keys = set(object_instance.get_fitted_params().keys())
    ...
    def test_pretrain_clone_preserves_state(self, object_instance, n_columns):
        ...
        pretrained_params_orig = object_instance.get_pretrained_params()
        assert len(pretrained_params_orig) > 0

        cloned = object_instance.clone()
    ...
    def test_pretrain_network_preserved_by_fit(self, object_instance, n_columns):
        ...
        if not hasattr(object_instance, "network"):
            return None

        net_id_before = id(object_instance.network)

        y_series = _make_series(n_columns=n_columns, n_timepoints=20)
        object_instance.fit(y_series, fh=fh)

        net_id_after = id(object_instance.network)
        assert net_id_before == net_id_after, (
            f"{object_instance.__class__.__name__}._fit() rebuilt the network "
