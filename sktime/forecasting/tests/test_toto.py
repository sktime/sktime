...
_TOTO_PROBA_DEPS = _check_estimator_deps(
    TotoForecaster, severity="none"
...
@pytest.mark.skipif(
    not _TOTO_PROBA_DEPS,
    reason="run test only if TotoForecaster and skpro dependencies are present",
)
...
def test_toto_predict_quantile_endpoints_match_empirical():
    """Toto's private quantile implementation handles alpha endpoints."""
    y = load_airline()
    y_train = y.iloc[:-16]
    fh = np.arange(1, 4)
    alpha = [0.0, 1.0]
    ...
    pred_dist = forecaster.predict_proba(fh=fh)
    pred_quantiles = forecaster._predict_quantiles(
        fh=forecaster.fh, X=None, alpha=alpha
    )
...
def test_toto_predict_proba_multivariate_targets():
    """Toto preserves target and sparse-horizon axes for multivariate y."""
    y = load_airline().iloc[:-16]
    y_train = pd.DataFrame(
        {"original": y, "scaled": 2 * y},
        index=y.index,
    )
    fh = np.array([1, 3, 5])
    forecaster = TotoForecaster(
