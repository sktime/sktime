...
class Splitting(nn_module):
    """
    A neural network module for splitting input into even and odd parts.

    The `Splitting` class defines a PyTorch module that splits an input
    tensor into two parts: the even-indexed and odd-indexed elements.
    This can be used in various tasks requiring input splitting or masking.
    """

    def __init__(self):
        """
        Initialize the Splitting module.

        This function initializes the `Splitting` class by calling the
        `__init__` method of its superclass (`nn.Module`). No additional
    ...
    def even(self, x):
        ...
        The function slices the input tensor to select even-indexed
        elements along the second dimension. The slicing uses a step
        of 2 starting from index 0.
        """
        return x[:, ::2, :]
    ...
    def odd(self, x):
        ...
        This function slices the input tensor to select odd-indexed
        elements along the second dimension, starting from index 1
        and stepping by 2.
        """
        return x[:, 1::2, :]
    ...
    def forward(self, x):
        ...
        The `forward` method applies the `even` and `odd` methods to
        split the input tensor into two parts. It returns both parts
        as a tuple: the even-indexed and odd-indexed elements.
        """
        return (self.even(x), self.odd(x))


class Interactor(nn_module):
    """
    ...
    def __init__(
        ...
        """
        Initialize the Interactor module with given parameters.

        in_planes (int): Number of input planes (channels).
        splitting (bool): Whether to split the input tensor.
        kernel (int): The kernel size for convolutions.
        dropout (float): Dropout rate for regularization.
        groups (int): Number of groups for convolutions.
        hidden_size (int): Size of the hidden layer.
        INN (bool): Whether to modify the network behavior.
        """
        super().__init__()
        self.modified = INN
...
class InteractorLevel(nn_module):
    """
    ...
    def __init__(self, in_planes, kernel, dropout, groups, hidden_size, INN):
        ...
        This method initializes the Interactor module with the provided
        parameters for input planes, kernel size, dropout, groups,
        hidden size, and the INN flag. It also sets up the required
...
class EncoderTree(nn_module):
    ...
    def __init__(
        ...
        num_levels (int): The number of hierarchical levels.
        kernel_size (int): The size of the convolutional kernel.
        dropout (float): Dropout rate.
        groups (int): Number of groups in convolution.
        hidden_size (int): Size of the hidden layer.
        INN (bool): Whether to use INN network architecture.
        """
        super().__init__()
            ...
            if self.stacks == 2:
                if self.concat_len:
                    self.projection2 = nn.Conv1d(
                        self.concat_len + self.pred_len, 1, kernel_size=1, bias=False
                    )
                else:
                    self.projection2 = nn.Conv1d(
