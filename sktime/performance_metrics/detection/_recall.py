"""Windowed recall metrics for event detection."""
...
class WindowedRecall(_BaseWindowedDetectionScore):
    r"""Windowed recall (sensitivity, hit rate, true positive rate).

    This score computes recall for point-event detection using one-to-one matching
    within a window around each true event.

    Denoting by :math:`M` the number of matched predicted/true event pairs and by
    :math:`|Y|` the number of true events, the metric is

    .. math::
        \mathrm{WindowedRecall} = \frac{M}{|Y|}.
