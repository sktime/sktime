...
def test_false_alarm_rate_unmatched_alarm():
    ...
    score = FalseAlarmRate(max_lead=2)(_events([5]), _events([1, 4]), X)
...
def test_false_alarm_rate_alarm_in_hit_window_is_not_false():
    ...
    score = FalseAlarmRate(max_lead=2)(_events([5]), _events([3, 4, 5]), X)
...
def test_false_alarm_rate_no_events():
    ...
    X = _make_X()
    score = FalseAlarmRate(max_lead=2)(_events([]), _events([2, 6]), X)
...
def test_false_alarm_rate_no_alarms():
    ...
    X = _make_X()
    score = FalseAlarmRate(max_lead=2)(_events([4, 7]), _events([]), X)
...
def test_false_alarm_rate_time_index():
    ...
    metric_min = FalseAlarmRate(max_lead=pd.Timedelta("20min"), time_unit="min")
...
def test_false_alarm_rate_integer_index():
    ...
    assert FalseAlarmRate(max_lead=10)(y_true, y_pred, X) == 1 / 40
    # time_unit has no effect without a time index
    metric = FalseAlarmRate(max_lead=10, time_unit="min")
...
def test_false_alarm_rate_uneven_time_spacing():
    ...
    metric = FalseAlarmRate(max_lead=pd.Timedelta("5s"), time_unit="s")
...
def test_false_alarm_rate_requires_X():
    ...
    metric = FalseAlarmRate(max_lead=2)
