...
def test_mean_advance_time_one_hit():
    ...
    X = _make_X()
    score = MeanAdvanceTime(max_lead=3)(_events([5]), _events([3]), X)
...
def test_mean_advance_time_miss_is_ignored():
    ...
    score = MeanAdvanceTime(max_lead=3)(y_true, y_pred, X)
...
def test_mean_advance_time_no_events():
    ...
    X = _make_X()
    score = MeanAdvanceTime(max_lead=3)(_events([]), _events([2]), X)
...
def test_mean_advance_time_no_alarms():
    ...
    X = _make_X()
    score = MeanAdvanceTime(max_lead=3)(_events([4, 7]), _events([]), X)
...
def test_mean_advance_time_no_hits():
    ...
    score = MeanAdvanceTime(max_lead=3)(_events([4, 7]), _events([0, 12]), X)
...
def test_mean_advance_time_time_index():
    ...
    metric = MeanAdvanceTime(max_lead=pd.Timedelta("4s"))
    assert metric(y_true, y_pred, X) == 3.0

    metric_ms = MeanAdvanceTime(max_lead=pd.Timedelta("4s"), time_unit="ms")
...
def test_mean_advance_time_integer_index():
    ...
    score = MeanAdvanceTime(max_lead=25)(y_true, y_pred, X)
...
def test_mean_advance_time_two_hits_take_earliest():
    ...
    score = MeanAdvanceTime(max_lead=4)(y_true, y_pred, X)
...
def test_mean_advance_time_requires_X():
    ...
    metric = MeanAdvanceTime(max_lead=3)
