...
def overlay_dx_score(
    ...
    **kwargs,
    ...
    tolerance_mode : {'range', 'quantile_range', 'absolute', 'relative'}, \
            default='range'
        How to define tolerance thresholds. See ``OverlayDX`` class for
        full details.

        * ``'range'``: global range-based (O(N log N + K)).
        * ``'quantile_range'``: robust interquantile-range-based (O(N log N + K)).
        * ``'absolute'``: raw units (O(N log N + K)).
        * ``'relative'``: per-point ``τ% × |y_true_i|`` (O(N × K)).
    ...
    sample_weight : array-like of shape (n_samples,), default=None
        Sample weights. Not currently supported; raises ``NotImplementedError``.
    ...
    score : float or ndarray
        ...
        ``ndarray`` if ``multioutput='raw_values'``.
    ...
    0.847
    >>> overlay_dx_score(y_true, y_pred, tolerance_mode='relative')  # doctest: +SKIP
    0.712
