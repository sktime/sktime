...
# alpha will be either s or m
# n_column will be either 1 (uni) or 3 (multi)
@pytest.fixture
def get_quantile_data(request):

    # get params for fixture
    n_columns, alpha = request.param
    ...
    # predict model
    quantile_pred = f.predict_quantiles(fh=fh, alpha=alpha)

    return y_test, quantile_pred

@pytest.fixture
def get_interval_data(request):

    n_columns, coverage = request.param
    ...
    # predict model
    interval_pred = f.predict_interval(fh=fh, coverage=coverage)
...
######## Test the parametrized fixtures ##############
# Use the parametric fixture with multiple parameters in a test
@pytest.mark.parametrize("get_quantile_data",
                         [(1, alpha_s), (3, alpha_s), (1, alpha_s), (3, alpha_s)], indirect=True)
def test_get_data_quantile(get_quantile_data):

    y_true, y_pred = get_quantile_data
    assert isinstance(y_true, (pd.Series, pd.DataFrame))
    assert isinstance(y_pred, pd.DataFrame)


# Use the parametric fixture with multiple parameters in a test
@pytest.mark.parametrize("get_interval_data",
                         [(1, coverage_s), (3, coverage_s), (1, coverage_m), (3, coverage_m)], indirect=True)
def test_get_data_interval(get_interval_data):
    y_true, y_pred = get_interval_data
    assert isinstance(y_true, (pd.Series, pd.DataFrame))
    assert isinstance(y_pred, pd.DataFrame)
...
# def test_output(metric, score_average, multioutput, get_data):
...
# This test tests the quantile data
@pytest.mark.parametrize("Metric", quantile_metrics)
@pytest.mark.parametrize("get_data",
                         [(1, alpha_s, 'quantile'), (3, alpha_s, 'quantile'),
                          (1, alpha_m, 'quantile'), (3, alpha_m, 'quantile')],
