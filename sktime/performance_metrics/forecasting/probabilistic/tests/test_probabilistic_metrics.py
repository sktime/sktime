...
# # this should be a parametric fixture with parameters s and m.
...
# Let's start out with a simple data type, interval_pred_uni_s

@pytest.fixture
def quantile_data(alpha, coverage):
    # create data
    y_uni = _make_series(n_columns=1)
    y_train_uni, y_test_uni = temporal_train_test_split(y_uni)
    fh_uni = np.arange(len(y_test_uni)) + 1
