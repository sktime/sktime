...
class CNTCRegressorTorch(BaseDeepRegressorTorch):
    ...
    self-attention and regresses with a multilayer perceptron, following [1]_.
    ...
    - **Multilayer perceptron.** Fully connected layers with dropout, followed
      by the output layer, which has a single unit for regression.
    ...
    activation : str, Callable, or None, default=None
        ...
        - ``None``: no activation is applied to the output layer and the network
          returns raw outputs. This is the usual choice for regression, where
    ...
    Examples
    --------
    >>> from sktime.regression.deep_learning.cntc import CNTCRegressorTorch
    >>> from sktime.datasets import load_unit_test
    >>> X_train, y_train = load_unit_test(split="train")
    >>> reg = CNTCRegressorTorch(num_epochs=5, batch_size=4)  # doctest: +SKIP
    ...
    def __post_init__(self):
        ...
        * any soft dependency imports in the constructor
        """
        # input_size is inferred from the data in _build_network
