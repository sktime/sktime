...
class CNTCRegressorTorch(BaseDeepRegressorTorch):
    ...
    >>> model = CNTCRegressorTorch(num_epochs=1, batch_size=2) # doctest: +SKIP
    >>> model.fit(X_train, y_train) # doctest: +SKIP
    CNTCRegressorTorch(...)
    >>> y_pred = model.predict(X_test) # doctest: +SKIP
    """
    ...
    def _build_network(self, X):
        ...
        ----------
        X: np.ndarray
            The input data, containing the timeseries data.
        ...
        # For regression the output size is 1 (continuous target)
        self.num_classes = 1
    ...
    def _predict(self, X):
        ...
        with torch.no_grad():
            for x1_batch, x3_batch in dataloader:
                batch_pred = self.network(x1=x1_batch, x3=x3_batch)
                batch_pred = batch_pred.detach().numpy()
                y_pred.append(batch_pred)

        return np.concatenate(y_pred, axis=0).squeeze(-1)
