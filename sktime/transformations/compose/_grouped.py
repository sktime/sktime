...
class GroupByCategoryTransformer(BaseTransformer, _HeterogenousMetaEstimator):
    """Apply transformers by time series category/cluster.

    Uses a categorizer to assign categories,
    then applies the corresponding transformer to each category group.

    Parameters
    ----------
    transformers : dict[str, sktime transformer]
        Dictionary of transformers with keys corresponding to categories
        generated by the categorizer.

    categorizer : sktime transformer or clusterer, default=ADICVTransformer (if not specified)
        Transformer that assigns categories to time series.

    fallback_transformer : sktime transformer or None, optional.
        Transformer used when category is not in ``transformers`` dict.
        If None and unknown category encountered, raises ValueError.

    Attributes
    ----------
    category_ : pd.Series
        Categories assigned to time series after fit.
    transformers_ : dict
        Fitted transformers, keyed by category.
