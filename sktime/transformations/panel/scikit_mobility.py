...
class ScikitMobilityFeatureExtractor(BaseTransformer):
    ...
    def _transform(self, X, y=None):
        ...
        try:
            ...
            from skmob.preprocessing import compression, detection, filtering
        except ImportError as e:
            # Check if it's a nested dependency issue
            error_msg = str(e)
            error_type = type(e).__name__

            # Extract missing module name if available
            missing_module = None
            if "No module named" in error_msg:
                # Extract module name from error message like "No module named 'requests'"
                import re
            ...
            if missing_module:
                raise ImportError(
                    f"scikit-mobility is installed but missing required dependency: '{missing_module}'. "
                    f"Install it with: pip install {missing_module}. "
                    f"Or reinstall scikit-mobility with all dependencies: pip install --upgrade scikit-mobility"
                ) from e
            else:
                raise ImportError(
                    f"scikit-mobility is required for ScikitMobilityFeatureExtractor but could not be imported. "
                    f"Error ({error_type}): {error_msg}. "
                    f"Install it with: pip install scikit-mobility"
                ) from e
        ...
        if "lat" in X_copy.columns and "lon" in X_copy.columns:
            ...
            X_copy.rename(columns={"latitude": "lat", "longitude": "lng"}, inplace=True)
        ...
        try:
            tdf = TrajDataFrame(
                X_copy, latitude="lat", longitude="lng", datetime="datetime"
            )
        ...
        if feature_list:
            ...
            if "tid" in features_df.columns or "tid" in tdf.columns:
                ...
                agg_dict = {
                    col: "mean" for col in features_df.columns if col != tid_col
                }
                agg_dict.update(
                    {
                        col: "sum"
    ...
    def _extract_speed_features(self, tdf):
        """Extract speed-related features (Vectorized)."""
        speed_features = pd.DataFrame()

        # Sort to ensure correct diff calculations
        tdf = tdf.sort_values(by=["tid", "datetime"])
        ...
        for tid, traj in tdf.groupby("tid"):
            ...
            # --- Vectorized Haversine Calculation (No loops!) ---
            # Convert to radians
            lat = np.radians(traj["lat"])
            lng = np.radians(traj["lng"])
            ...
            a = (
                np.sin(dlat / 2) ** 2
                + np.cos(lat.shift()) * np.cos(lat) * np.sin(dlng / 2) ** 2
            )
            c = 2 * np.arctan2(np.sqrt(a), np.sqrt(1 - a))
            dist_km = 6371 * c

            # Time diff in seconds
            time_diff = traj["datetime"].diff().dt.total_seconds()

            # Calculate Speed (m/s)
            # Handle division by zero or very small time deltas
            speeds = (dist_km * 1000) / time_diff
        ...
        return (
            speed_features.set_index("tid")
            if "tid" in speed_features.columns
            else speed_features
        )
    ...
    def _extract_distance_features(self, tdf):
        ...
        for tid, traj in tdf.groupby("tid"):
            ...
            if distances:
                ...
                distance_dict = {
                    ...
                    "efficiency_ratio": displacement_km / total_distance
                    if total_distance > 0
                    else 0,
                }
        ...
        return (
            distance_features.set_index("tid")
            if "tid" in distance_features.columns
    ...
    def _extract_direction_features(self, tdf):
        ...
        return (
            direction_features.set_index("tid")
            if "tid" in direction_features.columns
    ...
    def _extract_stop_features(self, tdf, duration_threshold, radius):
        ...
        try:
            ...
            for tid in tdf["tid"].unique():
                traj_stops = (
                    stops_tdf[stops_tdf["tid"] == tid]
                    if "tid" in stops_tdf.columns
                    else stops_tdf
                )
                ...
                stop_dict = {
                    ...
                    "stop_duration_total": traj_stops["duration"].sum()
                    if "duration" in traj_stops.columns
                    else 0,
                }
                stop_features = pd.concat(
                    [stop_features, pd.DataFrame([stop_dict])], ignore_index=True
                )

            return (
                stop_features.set_index("tid")
                if "tid" in stop_features.columns
                else stop_features
            )
        except Exception:
    ...
    def _extract_geometry_features(self, tdf):
        ...
        return (
            geometry_features.set_index("tid")
            if "tid" in geometry_features.columns
