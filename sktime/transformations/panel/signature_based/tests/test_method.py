...
def test_generalised_signature_method():
    ...
    # Check dyadic dim
    method = GeneralisedSignatureMethod(depth=depth, window_name='dyadic', window_depth=3)
    assert method.fit_transform(X).shape[1] == (esig.sigdim(n_channels + 1, depth) - 1) * 15

    # Ensure an example
    X = np.array([[0, 1], [2, 3], [1, 1]]).reshape(-1, 2, 3)
    method = GeneralisedSignatureMethod(depth=2, window_name='global')
    true_arr = np.array([[1., 2., 1., 0.5, 1.33333333, -0.5, 0.66666667, 2., -1., 1.5, 3., 0.5]])
...
def test_window_error():
    ...
    method = GeneralisedSignatureMethod(window_name='expanding', window_length=10, window_step=5)
    with pytest.raises(ValueError):
        method.fit_transform(X)
    method = GeneralisedSignatureMethod(window_name='sliding', window_length=10, window_step=5)
    with pytest.raises(ValueError):
        method.fit_transform(X)
