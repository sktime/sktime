...
class CalendarDummies(_SeriesToSeriesTransformer):
    ...
    base_frequency : str, optional (default=None)
        Restricts selection of items to those with frequeny higher than the base frequency.
        E.g. if monthly data is provided, it does not make sense to derive weekly dummies
        Currently has to be provided by the user due to the abundance of different frequencies supported by Pandas (e.g. every 4 days frequency)
        * year
        * quarter
        * month
        * week
    ...
    complexity: 0,1,2, optional (default =1)
        ...
        E.g., rarely used features like week of quarter will only be returned with complexity = 2.
    manual_selection:
        Manual selection of dummys. Notation is child of parent for precise notation.
        Will ignore specified complexity, but will still check with warning against a specified base_frequency 
        Examples:
        * day_of_year
        * day_of_month
        * day_of_quarter
    ...
    Example
    ----------
    >>> from sktime.transformations.series.impute import CalendarDummies
    ...
    def __init__(
        ...
        if (self.base_frequency is not None) & (self.base_frequency not in base_seasons["child"].unique()):
            raise ValueError("Invalid base_frequency specified, must be in: " + ', '.join(base_seasons["child"].unique()))

        self.complexity = complexity

        if self.complexity not in [0,1,2]:
            raise ValueError("Invalid complexity specified, must be in 0,1 or 2 (0 lowest number of variables)")
        self.manual_selection = manual_selection

        if (self.manual_selection is not None) and (not all(elem in base_seasons["dummy"].unique() for elem in self.manual_selection)):
            raise ValueError("Invalid manual_selection specified, must be in: " + ', '.join(base_seasons["dummy"].unique()))
    ...
    def transform(self, Z, X=None):
        ...
        if self.manual_selection is None:
            ...
            if self.base_frequency is not None:
                supported = _get_supported_calendar(self.base_frequency)
                if (not all(elem in supported["dummy"] for elem in self.manual_selection)):
                    warnings.warn("Selected dummy variable derive at lower level than base frequency.")
                calendar_dummies =  supported.loc[supported["dummy"].isin(self.manual_selection),"dummy_func"]
            else:
                calendar_dummies =  base_seasons.loc[base_seasons["dummy"].isin(self.manual_selection),"dummy_func"]
...
def _calendar_dummies(x,funcs):    
    ...
    if funcs == "week_of_year":
        ...
        #So it is possible that a week in the new year is still indexed starting in last year (week 52 or 53)
        x[funcs] =  getattr(date_sequence,"isocalendar")()["week"]
        return x[funcs]
        ...
        df['qdate'] = (df['date_sequence'] + pd.tseries.offsets.DateOffset(days=1)) - pd.tseries.offsets.QuarterBegin(startingMonth=1) 
        ...
        date_sequence.year.map(str) + "-" + (3 * quarter - 2).map(int).map(str) + "-01")
        values  = ((x["date_sequence"] - quarter_start) / pd.to_timedelta("1D") + 1).astype(int)
        x[funcs] =  values
        return x[funcs]
    else:
