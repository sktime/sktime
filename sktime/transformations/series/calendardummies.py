...
date_order = ['year', 'quarter', 'month', 'week', 'day',
...
base_seasons["child"] = base_seasons["child"].astype("category").cat.reorder_categories(date_order)
...
class CalendarDummies(_SeriesToSeriesTransformer):
    ...
    def __init__(
        ...
        if (self.base_frequency is not None) & (self.base_frequency not in base_seasons["child"].unique()):
            raise ValueError("Invalid base_frequency specified, must be in: " +
        ...
        if self.complexity not in [0, 1, 2]:
            raise ValueError(
                "Invalid complexity specified, must be in 0,1 or 2 (0 lowest number of variables)")
        ...
        if (self.manual_selection is not None) and (not all(elem in base_seasons["dummy"].unique() for elem in self.manual_selection)):
            raise ValueError("Invalid manual_selection specified, must be in: " +
        ...
        if self.manual_selection is None:
            ...
            if self.base_frequency is not None:
                ...
                if (not all(elem in supported["dummy"] for elem in self.manual_selection)):
                    warnings.warn(
                        "Selected dummy variable derive at lower level than base frequency.")
                calendar_dummies = supported.loc[supported["dummy"].isin(
                    self.manual_selection), "dummy_func"]
            else:
                calendar_dummies = base_seasons.loc[base_seasons["dummy"].isin(
                    self.manual_selection), "dummy_func"]
...
def _calendar_dummies(x, funcs):
    ...
    if funcs == "week_of_year":
        ...
        df['qdate'] = (df['date_sequence'] + pd.tseries.offsets.DateOffset(days=1)
        ...
        values = ((x["date_sequence"] - quarter_start) /
                  pd.to_timedelta("1D") + 1).astype(int)
        x[funcs] = values
        return x[funcs]
    else:
