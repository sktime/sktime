...
pipe = ForecastingPipeline(steps=[
    ("CalendarDummies", CalendarDummies(base_frequency="month",complexity=2,manual_selection=["year_of_year","second_of_minute"])),
    ("forecaster", NaiveForecaster(strategy="drift"))])
...
pipe = ForecastingPipeline(steps=[
    ("CalendarDummies", CalendarDummies(manual_selection=["year_of_year","second_of_minute"])),
    ("forecaster", NaiveForecaster(strategy="drift"))])
...
@pytest.mark.parametrize("test_input,expected", [
    (first_test,['GNPDEFL', 'GNP', 'UNEMP', 'ARMED', 'POP', 'year', 'quarter', 'month','week_of_year', 'month_of_quarter', 'week_of_quarter', 'week_of_month']),
    (second_test,["GNPDEFL","GNP","UNEMP","ARMED","POP","year","month"]),
    (third_test,['GNPDEFL', 'GNP', 'UNEMP', 'ARMED', 'POP', 'year', 'quarter', 'month', 'month_of_quarter']),
