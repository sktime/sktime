...
class TestAllTransformers(TransformerFixtureGenerator, QuickTester):
    ...
    def test_capability_inverse_tag_is_correct(self, object_instance):
        """Test that the capability:inverse_transform tag is set correctly."""
        capability_tag = object_instance.get_tag("capability:inverse_transform")
        skip_tag = object_instance.get_tag("skip-inverse-transform")
        if capability_tag and not skip_tag:
            assert object_instance._has_implementation_of("_inverse_transform")

    def test_remember_data_tag_is_correct(self, object_instance):
        """Test that the remember_data tag is set correctly."""
        fit_empty_tag = object_instance.get_tag("fit_is_empty", True)
        remember_data_tag = object_instance.get_tag("remember_data", False)
        msg = (
            ...
            f"Please check these two tags in {type(object_instance)}."
        )
    ...
    def test_fit_transform_output(self, object_instance, scenario):
        """Test that transform output is of expected scitype."""
        X = scenario.args["transform"]["X"]
        Xt = scenario.run(object_instance, method_sequence=["fit", "transform"])

        X_scitype = scenario.get_tag("X_scitype")
        trafo_input = object_instance.get_tag("scitype:transform-input")
        trafo_output = object_instance.get_tag("scitype:transform-output")
        ...
        msg = (
            f"{type(object_instance).__name__}.transform should return an object of "
        ...
        if type(object_instance).__name__ in ["Aggregator", "Reconciler"]:
            return None

        # if DataFrame is returned, columns must be unique
        if hasattr(Xt, "columns"):
            msg = (
                f"{type(object_instance).__name__}.transform return should have "
        ...
        if trafo_input == "Series" and trafo_output == "Series":
            if X_scitype == "Series" and Xt_scitype == "Series":
                if object_instance.get_tag("transform-returns-same-time-index"):
    ...
    def test_transform_inverse_transform_equivalent(self, object_instance, scenario):
        ...
        if not object_instance.get_class_tag("capability:inverse_transform", False):
            return None

        # skip this test if the estimator skips inverse_transform
        if object_instance.get_tag("skip-inverse-transform", False):
            return None

        # skip this test if inverse_transform is not assumed an exact inverse
        if not object_instance.get_tag("capability:inverse_transform:exact", True):
            return None

        X = scenario.args["transform"]["X"]
        Xt = scenario.run(object_instance, method_sequence=["fit", "transform"])
        Xit = object_instance.inverse_transform(Xt)

        # if inversion produces more indices, we subset to the original X indices
        if not object_instance.get_tag("transform-returns-same-time-index"):
        ...
        inv_range = object_instance.get_tag("capability:inverse_transform:range")
    ...
    def test_categorical_X_raises_error(self, object_instance):
        ...
        if not object_instance.get_tag("capability:categorical_in_X"):
            with pytest.raises(TypeError, match=r"categorical"):
                object_instance.fit_transform(X, y)
    ...
    def test_categorical_y_raises_error(self, object_instance):
        ...
        if requires_y or uses_y:
            if not object_instance.get_tag("capability:categorical_in_y"):
                with pytest.raises(TypeError, match=r"categorical"):
                    object_instance.fit_transform(X, y)
        ...
        elif not object_instance.is_composite():
            object_instance.fit_transform(X, y)
    ...
    def test_categorical_X_passes(self, object_instance):
        ...
        if (
            ...
            and not object_instance.is_composite()
        ):
            try:
                object_instance.fit_transform(X, y)
            except Exception as e:
                raise RuntimeError(
                    f"{object_instance} fails when passing categorical X, "
