...
import numpy as np
import pytest
from sktime.transformers.detrend import ResidualDetrender, Deseasonaliser
...
@pytest.mark.parametrize("Detrender", DETRENDERS)
def test_transform_time_index(Detrender):
    t = _construct_instance(Detrender)
    t.fit(y_train)
...
@pytest.mark.parametrize("Detrender", DETRENDERS)
def test_inverse_transform_time_index(Detrender):
    t = _construct_instance(Detrender)
    t.fit(y_train)
...
@pytest.mark.parametrize("Detrender", DETRENDERS)
def test_transform_inverse_transform_equivalence(Detrender):
    t = _construct_instance(Detrender)
    t.fit(y_train)
