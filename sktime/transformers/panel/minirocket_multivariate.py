...
@njit("float32[:](float32[:,:,:],int32[:],int32[:],int32[:],int32[:],float32[:],optional(int32))", fastmath = True, parallel = False)
def _fit_biases_multi(X, num_channels_per_combination, channel_indices, dilations, num_features_per_dilation, quantiles, seed):
    ...
    for dilation_index in range(num_dilations):
        ...
        for kernel_index in range(num_kernels):
            ...
            C_alpha = np.zeros((num_channels_this_combination, n_timepoints), dtype = np.float32)
            C_alpha[:] = A

            C_gamma = np.zeros((9, num_channels_this_combination, n_timepoints), dtype = np.float32)
            C_gamma[9 // 2] = G
...
def _fit_multi(X, num_features = 10_000, max_dilations_per_kernel = 32, seed = None):
    ...
    for combination_index in range(num_combinations):
        num_channels_this_combination = num_channels_per_combination[combination_index]
        num_channels_end = num_channels_start + num_channels_this_combination
        channel_indices[num_channels_start:num_channels_end] = np.random.choice(n_columns, num_channels_this_combination, replace = False)

        num_channels_start = num_channels_end

    biases = _fit_biases_multi(X, num_channels_per_combination, channel_indices, dilations, num_features_per_dilation, quantiles, seed)
