...
def _generate_kernels(n_timepoints, num_kernels, n_columns, seed):
    ...
    weights = np.zeros(np.int32(np.dot(lengths.astype(np.float64), num_channel_indices.astype(np.float64))),
                       dtype=np.float64)
...
@njit("float64[:,:](float64[:,:,:],Tuple((float64[::1],int32[:],float64[:],int32[:],int32[:],int32[:],int32[:])))",
    ...
    for i in prange(n_instances):
        ...
        for j in range(num_kernels):
            ...
            if num_channel_indices[j] == 1:
                ...
                _X[i, a3:b3] = \
                    _apply_kernel_univariate(X[i, channel_indices[a2]], weights[a1:b1], lengths[j], biases[j],
                ...
                _X[i, a3:b3] = \
                    _apply_kernel_multivariate(X[i], _weights, lengths[j], biases[j], dilations[j], paddings[j],
                                               num_channel_indices[j], channel_indices[a2:b2])
