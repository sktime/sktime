...
def handle_sktime_signatures(check_fitted=False):
    ...
    This decorator assumes that the input arguments to the function are either of the form:
        (self, data, labels)
    or
        (self, data).

    If this is in sktime format data, it will check the data and labels are of the correct form, and then
    """
    def real_decorator(func):
        """ Reusable function to handle the sktime checks and convert the data to a torch Tensor. """
        ...
        def wrapper(*args, **kwargs):
            ...
            if len(args) == 2:
                ...
                raise NotImplementedError("This decorator can only be used for args (self, data, labels) "
                                          "or (self, data).")
            # Fit checks
            if check_fitted:
...
def sktime_to_tensor(data):
    """ Signature functionality requires torch tensors. This converts from the sktime format. """
