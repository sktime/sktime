...
class GeneralisedSignatureMethod(BaseSeriesAsFeaturesTransformer):
    ...
    augmentation_list: list of strings, List of augmentations to be applied before the signature transform is applied.
    window_name: str, The name of the window transform to apply.
    window_kwargs: dict, Additional parameters to be supplied to the window method.
    rescaling: str, The method of signature rescaling.
    sig_tfm: str, String to specify the type of signature transform. One of: ['signature', 'logsignature']).
    depth: int, Signature truncation depth.

    Attributes
    ----------
    signature_method: sklearn.Pipeline, A sklearn pipeline object that contains all the steps to extract the signature
        features.
    """
    def __init__(self,
                 scaling='stdsc',
    ...
    def setup_feature_pipeline(self):
        ...
        transform_step = WindowSignatureTransform(
            self.window_name, self.window_kwargs, self.sig_tfm, self.depth, rescaling=self.rescaling
        )
