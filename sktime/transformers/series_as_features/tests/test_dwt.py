...
def test_output_of_transformer():
    ...
    res = d.transform(X)
    orig = convert_list_to_dataframe([[16,12,-6,2,-math.sqrt(2),-math.sqrt(2),math.sqrt(2),0]])
    ...
    d = d.fit(X)
    res = d.transform(X)
    orig = convert_list_to_dataframe([[0.75000,13.25000,-3.25000,-4.75000,-5.303301,-1.414214,8.131728,-4.242641]])
...
def test_dwt_performs_correcly_along_each_dim():
    ...
    res = d.transform(X)
    orig = convert_list_to_dataframe([[9*math.sqrt(2),-4*math.sqrt(2),-2,-2,-math.sqrt(2)/2,-math.sqrt(2)/2,-math.sqrt(2)/2,-math.sqrt(2)/2,-math.sqrt(2)/2],
                                      [9*math.sqrt(2),-4*math.sqrt(2),-2,-2,-math.sqrt(2)/2,-math.sqrt(2)/2,-math.sqrt(2)/2,-math.sqrt(2)/2,-math.sqrt(2)/2]])
    orig.columns = X.columns
    assert check_if_dataframes_are_equal(res,orig)
