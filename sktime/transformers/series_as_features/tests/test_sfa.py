...
from sktime.utils.data_container import tabularize, detabularise
...
def test_transformer():
    ...
    assert np.equal(0, p.breakpoints[1,:-1]).all() # imag component should be 0
    ...
    assert np.equal(0, p.breakpoints[1,:-1]).all() # imaginary component should be 0
...
def test_dft_mft():
    ...
    for norm in [True, False] :
        for window_size in [140] :
            p = SFA(word_length=word_length, norm=norm, alphabet_size=alphabet_size,
                    window_size=window_size, binning_method="equi-depth").fit(X, Y)
            mft = p._mft(X_tab[0])
            for i in range(len(X_tab[0]) - window_size + 1) :
                dft_transformed = p._discrete_fourier_transform(X_tab[0, i:window_size+i])
...
#     print("Single DFT transformation")
#     window_size = np.shape(X_tab)[1]
#     p = SFA(word_length=word_length, alphabet_size=alphabet_size, window_size=window_size,
#             binning_method="equi-depth").fit(X, Y)
#     dft = p._discrete_fourier_transform(X_tab[0])
#     print(p._create_word(dft))
...
def test_sfa_anova():
    ...
    for binning in ["information-gain","equi-depth"] : #
        ...
        window_size = 32
        p = SFA(word_length=word_length, anova=True, alphabet_size=alphabet_size,
        ...
        p2 = SFA(word_length=word_length, anova=False, alphabet_size=alphabet_size,
                 window_size=window_size,
                binning_method=binning).fit(X, Y)
            ...
            for norm in [True, False]:
                for anova in [True, False]:
                    for window_size in window_sizes :
                        p = SFA(word_length=word_length, anova=anova, alphabet_size=alphabet_size,
                                window_size=window_size, norm=norm,
                                binning_method=binning).fit(X, Y)

                        print ("Norm", norm, "Anova", anova)
                        print(np.shape(p.breakpoints), word_length, window_size)
                        print("dft_length", p.dft_length, "word_length", p.word_length)
                        #assert(np.shape(p.breakpoints)==min(word_lengths, window_sizes))
