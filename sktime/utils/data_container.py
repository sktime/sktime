...
def from_multi_index_to_3d_numpy(
    X, instance_index = None, time_index = None
):
...
def from_3d_numpy_to_multi_index(
    ...
    """
    if X.ndim != 3:
        msg = ' '.join(['Input should be 3-dimensional NumPy array with shape',
...
def from_multiindex_to_nested(
    ...
    instance_idxs = (multi_ind_dataframe
                        .index
                        .get_level_values(instance_index)
                        .unique()
    )
    ...
    for _label, _series in multi_ind_dataframe.iteritems():
        ...
        if cells_as_numpy:
            dim_list = [_series.xs(instance_idx, 
                                    level = instance_index).values
                            for instance_idx in instance_idxs]
        else:
            dim_list = [_series.xs(instance_idx, 
                                   level = instance_index).rename_axis(None) 
                            for instance_idx in instance_idxs]
...
def from_nested_to_multi_index(
    ...
    n_columns = X.shape[1]
    nested_col_mask = [_is_nested_structure(X.iloc[0, j]) 
    ...
    instances = []
    for instance_idx in instance_idxs:
        instance = [_val if isinstance(_val, pd.Series)  
                        else pd.Series(_val, name = _lab)
...
def from_nested_to_3d_numpy(X):
    ...
    cell_lambda = (lambda cell: cell.to_numpy() 
                            if isinstance(cell, pd.Series) else cell)
    # If all the columns are nested in structure
    if nested_col_mask.count(True) == len(nested_col_mask):     
        X_3d = np.stack(
            X.applymap(cell_lambda)
...
def from_3d_numpy_to_nested(X, column_names = None, cells_as_numpy = False):
    ...
    if column_names is None:
        ...
        if len(column_names) != n_columns:
            msg = ' '.join([f"Input 3d Numpy array as {n_columns} columns,",
                            f"but only {len(column_names)} names supplied"])
            raise ValueError(msg)
  
    for column in enumerate(column_names):
        df[column] = [container(X[instance, column, :]) 
...
def nested_dataframes_equal(X1, X2):
    ...
    if X1.shape != X2.shape:
        ...
        x1_nested_col_mask = [_is_nested_structure(X1.iloc[0, j]) 
                                for j in range(n_columns)]

        x2_nested_col_mask = [_is_nested_structure(X2.iloc[0, j]) 
                                for j in range(n_columns)]
            ...
            for j, is_nested in enumerate(x1_nested_col_mask):
                    ...
                    for i in range(n_instances):
                        ...
                        if x1_numeric != x2_numeric:
                            cell_is_same[i, j] = False
                                ...
                                cell_is_same[i, j] = np.isclose(X1.iloc[i, j], 
                                                           X2.iloc[i, j]).all()
                ...
                # Compare all instances in primitive columns at same time                    
                    ...
                    if x1_numeric != x2_numeric:
                        ...
                        cell_is_same[:, j] = np.isclose(X1.iloc[:, j], 
                                                        X2.iloc[:, j]).all()
