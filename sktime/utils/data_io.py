...
Loading:
Contains functions for loading data from alternative file formats into
the pandas data structure used by sktime.

the load_from files assume that the data is already formatted by case.
This means every row is an instance/case, and if there is a class label it
is the last value in the row. An notebook for usage

.ts is our own data format. A full description of it is
LINK.
.arff is the Weka format. An overview of weka format and how we adapt
it for multivariate is here
LINK
...
TO DO:
1. complete these comments and associated notebooks
2. test with timestamps
3. include meta information in the dataframe
4. provide tools for formatting from long format into a
classification/clustering problem
5. sort out use cases for saving to file
"""
__all__ = ["load_from_tsfile_to_dataframe",
