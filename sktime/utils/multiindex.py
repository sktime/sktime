...
def is_hierarchical(multiindex: pd.Index, raise_if_false=False) -> bool:
    """
    Determines whether a pandas MultiIndex is hierarchical, meaning
    each lower-level value corresponds to only a single higher-level value.
    ...
    for level in range(len(multiindex.levels) - 1):
        ...
        mapping=pd.DataFrame({f'{level}':parent, f'{level+1}':child}).drop_duplicates()
