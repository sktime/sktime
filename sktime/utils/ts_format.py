...
def write_df_to_data(ts_labels, df, out_path):
    '''
    ...
    for key in df:
        ...
        for dict_key in df_dict:
            ...
            data_arr = df_dict[dict_key].to_list()
            if (new_line != True):
                data += ":"
            new_line = False
            for val in data_arr:
...
def csv_to_ts_format(problem_name, path_to_csv, out_path, 
                     group_by, class_label_column = None, timestamps='false'):
    '''
    ...
    write_df_to_data(ts_label, df, out_path)
