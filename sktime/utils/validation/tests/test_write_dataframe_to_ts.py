...
from sktime.utils.results_writing import write_dataframe_to_tsfile


def test_write_dataframe_to_ts_success(tmp_path):
	# load an example dataset
	path = os.path.join(os.path.dirname(sktime.__file__), "datasets/data/ItalyPowerDemand/ItalyPowerDemand_TEST.ts")
	test_x, test_y = load_from_tsfile_to_dataframe(path)
	# output the dataframe in a ts file
	write_dataframe_to_tsfile(data=test_x, path=tmp_path, problem_name="ItalyPowerDemand",
	                          timestamp=False, univariate=True, class_label=[1,2], class_value_list=test_y,
	                          comment='''
...
def test_write_dataframe_to_ts_fail():
	with pytest.raises(ValueError, match="Data provided must be a DataFrame"):
		write_dataframe_to_tsfile(data=np.random.rand(3, 2), path=str(tmp_path), problem_name="GunPoint",
